      ...
      <div className="mx-auto flex max-w-6xl flex-row-reverse flex-wrap items-center justify-between gap-3 border-b border-black/5 bg-white px-4 py-2 text-xs text-slate-600">
        ...
        <div className="flex items-center gap-3">
          ...
          {isAuthenticated && user ? (
            <>
              <span className="text-slate-700 text-[11px]">{user.name}</span>
              {user.role === "vendor" && (
                <Link
                  href="/vendor/buses"
                  className="rounded-md border border-black/10 px-3 py-1 text-[11px] font-semibold text-slate-700 hover:bg-blue-50"
                >
                  داشبورد
                </Link>
              )}
              {user.role === "admin" && (
                <Link
                  ...
                  className="rounded-md border border-black/10 px-3 py-1 text-[11px] font-semibold text-slate-700 hover:bg-purple-50"
                >
                  ادمین
                </Link>
              )}
              <button
                ...
                className="rounded-md border border-black/10 px-3 py-1 text-[11px] font-semibold text-slate-700 hover:bg-red-50"
              >
                خروج
              </button>
            </>
